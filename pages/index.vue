...
const fetchData = async () => {
  ...
  try {
    ...
    if (txRes.status === "fulfilled") transactions.value = (txRes.value as any) || [];
    if (catRes.status === "fulfilled") categories.value = (catRes.value as any) || [];
  } catch {}
  loading.value = false;
};
...
<template>
  <div class="min-h-screen bg-canvas">
    ...
    <!-- Top nav -->
    <nav class="border-b border-line bg-white/80 backdrop-blur-sm sticky top-0 z-20">
      <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          <span class="text-sm text-ink-muted hidden sm:block">{{ monthName }} {{ new Date().getFullYear() }}</span>
          <button
    ...
    <main class="max-w-6xl mx-auto px-6 py-8">
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <!-- Transactions -->
          <div class="lg:col-span-3 animate-fade-up" style="animation-delay:0.2s; opacity:0">
            <div class="bg-white rounded-2xl shadow-subtle border border-line overflow-hidden">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <!-- Categories -->
            <div class="bg-white rounded-2xl shadow-subtle border border-line p-6 animate-fade-up" style="animation-delay:0.3s; opacity:0">
              <h2 class="font-display text-lg text-ink mb-4">Kategorie</h2>
              <div v-if="categories.length === 0" class="text-xs text-ink-muted text-center py-4">
                Žádné kategorie
              </div>
